...
#ifdef TP_MULTICAST_MARK
static atomic_t lan_dev_id;
static void br_multicast_port_group_leave_expired(unsigned long data);
#endif

static void br_multicast_leave_group(struct net_bridge *br,
				     struct net_bridge_port *port,
				     __be32 group, u32 skb_mark);
...
#ifdef TP_MULTICAST_MARK
    /*
    if(is_lan_dev(port->dev)) {
        skb->mark = pg->member_mark;
    }
    */
#endif
	if (!skb)
...
static int br_multicast_add_group(struct net_bridge *br,
				  struct net_bridge_port *port, __be32 group, __u32 skb_mark)
{
	struct net_bridge_mdb_entry *mp;
	struct net_bridge_port_group *p;
	struct net_bridge_port_group **pp;
...
#ifdef TP_MULTICAST_MARK
    if(is_lan_dev(p->port->dev)) {
        setup_timer(&p->leave_timer, br_multicast_port_group_leave_expired,
                    (unsigned long)p);
    }
#endif
...
#ifdef TP_MULTICAST_MARK
    if(is_lan_dev(p->port->dev)) {
        p->member_mark |= (skb_mark | IGMP_DATA_DOWN_FLAG);
        p->delete_mark &= (~skb_mark);
    }
#endif
				     ...
				     struct sk_buff *skb)
{
	...
	for (i = 0; i < num; i++) {
		...
		if ((type == IGMPV3_CHANGE_TO_INCLUDE ||
            ...
            br_multicast_leave_group(br, port, group, skb->mark);
		} else {
            err = br_multicast_add_group(br, port, group, skb->mark);
			if (err)
				break;
		}
	}
	...
	for (p = mp->ports; p; p = p->next) {
		...
		if (!hlist_unhashed(&p->mglist) &&
            ...
            if (is_lan_dev(p->port->dev)) {
                time += 10;
                p->delete_mark |= skb_mark;
                if(timer_pending(&p->leave_timer) ?
                 time_after(p->leave_timer.expires, time) :
                 try_to_del_timer_sync(&p->leave_timer) >= 0) {
                    mod_timer(&p->leave_timer, time);
                }
            }
#endif
		}
				 ...
				 struct sk_buff *skb)
{
	...
	case IGMPV2_HOST_MEMBERSHIP_REPORT:
		BR_INPUT_SKB_CB(skb)->mrouters_only = 1;
		err = br_multicast_add_group(br, port, ih->group, skb->mark);
		break;
	...
	case IGMP_HOST_LEAVE_MESSAGE:
		br_multicast_leave_group(br, port, ih->group, skb->mark);
		break;
	}
...
static void br_multicast_port_group_leave_expired(unsigned long data)
{
    ...
    if (!pg || !pg->port || !pg->port->br) {
        return ;
    }
    br = pg->port->br;

	spin_lock(&br->multicast_lock);
    pg->member_mark &= (~(pg->delete_mark));
    pg->delete_mark = 0;
	spin_unlock(&br->multicast_lock);
}

#define get_last_addr_byte(dev) ((dev)->dev_addr[(dev)->addr_len - 1])
bool is_lan_dev(struct net_device *dev) 
{
    return get_last_addr_byte(dev) == (atomic_read(&lan_dev_id) & 0xff);
}

void br_multicast_update_lan_dev_id(char id)
{
    printk("[br_multicast] update lan dev id to %02x\n", id);
    atomic_set(&lan_dev_id, id);
}
EXPORT_SYMBOL(br_multicast_update_lan_dev_id);
#endif
