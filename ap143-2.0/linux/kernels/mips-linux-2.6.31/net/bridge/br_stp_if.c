...
void br_stp_recalculate_bridge_id(struct net_bridge *br)
{
	...
	const unsigned char *eth1_addr = br_mac_zero;
	...
	list_for_each_entry(p, &br->port_list, list) {
		...
		else if (!strcmp(p->dev->name, "eth1"))	/* record eth1's mac */
			eth1_addr = p->dev->dev_addr;
	...
	if (ath0_addr != br_mac_zero)		/* if ath0 exists, use ath0's mac */
		...
		addr = eth0_addr;
	else if (eth1_addr != br_mac_zero)	/* no eth0, use eth1's mac */
		addr = eth1_addr;
