...
/* added by ZQQ, 17Sep13 */
#ifdef CONFIG_IPV6_ROUTER_WAN_SLAAC	
	.slaac_addr		= "",
#endif
/* end add by ZQQ, 17Sep13 */
/* added by ZQQ, 2015.3.5*/
	.dad_result		= 0,
/*end added by ZQQ*/
	.wan_dev_name 	= "eth1"
};
	      ...
	      int scope, u32 flags)
{
	...
	idev->cnf.dad_result = 0;
	ifa->rt = rt;
	//printk("%s %d: add address %pI6, prefix %d\r\n", __FUNCTION__, __LINE__, addr, pfxlen);
	/*
...
/* added by ZQQ, 17Sep13 */
#ifdef CONFIG_IPV6_ROUTER_WAN_SLAAC
			//printk("%s %d %pI6, %d\r\n", __FUNCTION__, __LINE__, &addr, pinfo->prefix_len);
...
static void addrconf_dad_completed(struct inet6_ifaddr *ifp)
{
	...
	if ((ifp->idev->cnf.forwarding == 0 &&
		...
		(dev->flags&IFF_LOOPBACK) == 0 &&
	    (ipv6_addr_type(&ifp->addr) & IPV6_ADDR_LINKLOCAL)) 
	    || (ifp->idev->cnf.forwarding == 1 && 0 == strcmp(ifp->idev->dev->name, ipv6_devconf_dflt.wan_dev_name))) {
		/*
...
/* added by ZQQ, 17Sep13 */
		...
		{
			//.ctl_name	=	CTL_UNNUMBERED,
			.procname	=	"wan_dev_name",			
			.data		=	&ipv6_devconf.wan_dev_name,
			.maxlen		=	16,			
			.mode		=	0644,			
			.proc_handler	=	proc_dostring,
		},
