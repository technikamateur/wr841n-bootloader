...
static void ndisc_recv_na(struct sk_buff *skb)
{
	...
	if (ifp) {
		...
		{
			addrconf_dad_failure(ifp);
			/*added by ZQQ, 2015.3.4*/
			printk ("%s %d: IPv6 addr dad fail.\r\n", __FUNCTION__, __LINE__);
			if (NULL != in6_dev)
			{
				in6_dev->cnf.dad_result = 1;
				printk ("%s %d: IPv6 addr dad, set in6_dev->cnf.dad_result = %d.\r\n", __FUNCTION__, __LINE__, in6_dev->cnf.dad_result);
			}
			/*end added by ZQQ.*/
		...
		else
		{
			if (NULL != in6_dev)
			{
				printk ("%s %d: IPv6 addr dad OK.\r\n", __FUNCTION__, __LINE__);
				in6_dev->cnf.dad_result = 0;
				printk ("%s %d: IPv6 addr dad, set in6_dev->cnf.dad_result = %d.\r\n", __FUNCTION__, __LINE__, in6_dev->cnf.dad_result);
			}
		}
	...
	if (neigh) {
		...
		if ((old_flags & ~neigh->flags) & NTF_ROUTER) {
			...
			/* delete the default when get na, but if it is wan device, it can not do that. by ZQQ, 22Dec13 */
...
/*end from ap136 kernel*/
        /* changed by ZQQ, 06Nov13 */
#if 0
...
/* end changed by ZQQ, 30Mar14 */
	...
	/*
	if ((in6_dev->cnf.forwarding || !in6_dev->cnf.accept_ra))
		//&& (0 != strcmp(in6_dev->dev->name, net->ipv6.devconf_dflt->wan_dev_name)))
	{
		//printk("%s %d goto skip_linkparms\r\n", __FUNCTION__, __LINE__); 
		goto skip_linkparms;
	}
	*/
...
#ifdef CONFIG_IPV6_MO_BIT
	__u32 moflag = (ra_msg->icmph.icmp6_addrconf_managed ? IF_RA_MANAGED : 0) | (ra_msg->icmph.icmp6_addrconf_other ? IF_RA_OTHERCONF : 0);
	/*added by ZQQ, 2015.11.11, to support M/O bit, see RFC 4861*/
	printk("%s %d: ra_msg->icmph.icmp6_addrconf_managed = %d, ra_msg->icmph.icmp6_addrconf_other = %d, lifetime = %d, in6_dev->if_flags = 0x%x\r\n\r\n", __FUNCTION__,__LINE__, 
		ra_msg->icmph.icmp6_addrconf_managed, ra_msg->icmph.icmp6_addrconf_other, lifetime, in6_dev->if_flags);
	printk("%s %d write to if_flags = 0x%x\r\n", __FUNCTION__,__LINE__, moflag & 0xFF);
	//sprintf(&in6_dev->cnf.if_flags, "%x", (moflag & 0xFF));
	in6_dev->cnf.if_flags = (moflag & 0xFF);
	/*end added by ZQQ.*/
#endif
