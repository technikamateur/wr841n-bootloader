...
ppp_asynctty_open(struct tty_struct *tty)
{
	...
	for ( i = 0; i < PKTNUM; i ++)
	{
		strcpy(ap->pkts[i].magic, "tp_magic");
	}
	...
	ap->raccm = ~0U;
	ap->optr = ap->pkts[ap->pkt_pos].obuf;
	ap->olim = ap->pkts[ap->pkt_pos].obuf;
	ap->lcp_fcs = -1;
...
int get_free_buf(struct asyncppp *ap)
{
	int i;

	for( i = 0; i < PKTNUM; i ++)
	{
		if (ap->pkts[i].in_use == 0)
		{
			ap->pkt_pos = i;
			//printk("pkt_pos is %d \n", ap->pkt_pos);
			return 1;
	...
	printk("fatal error: &&&&&&&&&&&&&&&&&& can not get free buffer in ppp\n");
	return 0;
}
...
ppp_async_encode(struct asyncppp *ap)
{
	...
	if (!get_free_buf(ap))
	{
		printk("&&&&&& can not get free ppp packet buffer!!\n");
		return 0;
	}

	buf = ap->pkts[ap->pkt_pos].obuf;
	ap->olim = buf;
	ap->optr = buf;
	i = ap->tpkt_pos;
	data = ap->tpkt->data;
	 ...
	 */
	buflim = ap->pkts[ap->pkt_pos].obuf + OBUFSIZE - 6;
