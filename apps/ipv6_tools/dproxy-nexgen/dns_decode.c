...
{
  struct in6_addr *addr;    /* by wdl, 03Mar11, change to support ipv6 */
  ...
  if( m->message.question[0].type == PTR ){
      strncpy( m->ip, m->message.question[0].name, len );
  ...
  for( i = 0; i < m->message.header.ancount && i < NUM_RRS; i++){
     ...
     if( m->message.question[0].type == m->message.answer[i].type ){
         if( m->message.answer[i].type == A ){
             /* Standard lookup so convert data to an IP(ipv4) */
             ipv4Addr = (struct in_addr *)m->message.answer[i].data;
             inet_ntop(AF_INET, &ipv4Addr, m->ip, len );    /* by wdl, 02Mar11, use inet_ntop to change ip */
             break;
         }
         else if( m->message.answer[i].type == AAAA )
         {
             /* by wdl, 03Mar11, Standard lookup so convert data to an IP(ipv6) */
             addr = (struct in6_addr *)m->message.answer[i].data;
             inet_ntop(AF_INET6, &addr, m->ip, len );
             break;
         }
         else if( m->message.answer[i].type == PTR )
         {
             ...
             strncpy( m->ip, m->message.answer[i].name, len );
             break;
         }
     } /* if( question == answer ) */
  } /* for */
