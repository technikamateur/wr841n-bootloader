...
void death_handler (int signal)
{
    ...
    /* added by kuangguiming, if xl2tpd received signal TERM, kill pppd here, cancel 'findSystemProc(L2TP_PROC_PPPD, 15)' in httpd)*/
	pid_t pid;
	struct call *c;
	...
	if (signal == SIGTERM)
	{
		...
		while (st)
		{
			...
			{
				kill(c->pppd, SIGTERM);
				//log_to_web ("%s: called kill(c->pppd, SIGTERM), pppd=%d", __FUNCTION__, c->pppd);
				c = c->next;
			}
		}
	}
	
	sleep(1);  /* wait pppd close its connection */
    /* end add by kuangguiming 14/03/17 */

	/*if (signal != SIGTERM) {*/    //delete by kuangguiming 14/03/17
    st = tunnels.head;
    while (st)
    {
        st2 = st->next;
...
int start_pppd (struct call *c, struct ppp_opts *opts)
{
	...
	/* added by ZQQ, fix bug 52127, httpd find pppd pid in /var/run/ppp0.pid, 
	  if we do not pass this parameter, pppd will create pid file as ppp-1.pid, 
	  in that case, httpd can not find ppp pid by findSystemProcByPidFile() */
