...
static void ctrlp_rep( void * buffer, int size, int isbuff)
{
	...
	if (type != PPTP_ECHO_RPLY)
		log_to_web(LOG_INFO, "%s %s", isbuff ? "buffered": "sent", ctrl_msg_types[type <= MAX_CTRLMSG_TYPE ? type : 0]);
#else
...
int ctrlp_disp(PPTP_CONN * conn, void * buffer, size_t size)
{
    ...
    if (size < PPTP_CTRL_SIZE(ntoh16(header->ctrl_type))) {
		#ifdef TP_WEB_LOG
		log_to_web(LOG_INFO, "rcvd invalid packet [type=%d length=%d]", 
    ...
    switch (ntoh16(header->ctrl_type)) {
        ...
        case PPTP_START_CTRL_CONN_RQST:
        {
			...
			#ifdef TP_WEB_LOG
			log_to_web(LOG_INFO, "rcvd Start-Control-Connection-Request");
			#else
        ...
        case PPTP_START_CTRL_CONN_RPLY:
        {
			...
			#ifdef TP_WEB_LOG
			log_to_web(LOG_INFO, "rcvd Start-Control-Connection-Reply");
        ...
        case PPTP_STOP_CTRL_CONN_RQST:
        {
			...
			#ifdef TP_WEB_LOG
			log_to_web(LOG_INFO, "rcvd Stop-Control-Connection-Request");
			#else
        ...
        case PPTP_STOP_CTRL_CONN_RPLY:
        {
			...
			#ifdef TP_WEB_LOG
			log_to_web(LOG_INFO, "rcvd Stop-Control-Connection-Reply");
			#else
        ...
        case PPTP_OUT_CALL_RQST:
        {
			...
			#ifdef TP_WEB_LOG
			log_to_web(LOG_INFO, "rcvd Outgoing-Call-Request");
			#else
        ...
        case PPTP_OUT_CALL_RPLY:
        {
			...
			#ifdef TP_WEB_LOG
			log_to_web(LOG_INFO, "rcvd Outgoing-Call-Reply");
			#else
        ...
        case PPTP_CALL_CLEAR_RQST:
        {
			...
			#ifdef TP_WEB_LOG
			log_to_web(LOG_INFO, "rcvd Call-Clear-Request");
			#else
        ...
        case PPTP_CALL_CLEAR_NTFY:
        {
			...
			#ifdef TP_WEB_LOG
			log_to_web(LOG_INFO, "rcvd Call-disconnect-notification");
			#else
        ...
        case PPTP_SET_LINK_INFO:
        {
			...
			#ifdef TP_WEB_LOG
			log_to_web(LOG_INFO, "rcvd PPTP-Set-Link-Info");
			#else
        ...
        default:
			...
			#ifdef TP_WEB_LOG
			log_to_web(LOG_INFO, "rcvd Unrecognized Packet");
			#else
