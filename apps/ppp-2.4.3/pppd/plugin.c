...
#ifdef MODULE_HWNAT_AP152
    sprintf(buf,"echo %d > /proc/qca_switch/nf_athrs17_hnat_ppp_id", conn->session);
#else
...
#ifdef MODULE_HWNAT_AP152
	sprintf(buf,"echo %02x%02x%02x%02x%02x%02x > /proc/qca_switch/nf_athrs17_hnat_ppp_peer_mac",
...
#ifdef MODULE_HWNAT_AP152
	sprintf(buf,"echo %02x%02x%02x%02x%02x%02x > /proc/qca_switch/nf_athrs17_hnat_wan_mac",
