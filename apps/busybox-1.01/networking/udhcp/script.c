...
#ifdef MODULE_OPTION66
#define OPTION66_TFTP_IP_LEN	(16)
typedef struct _OPTION66_CFG
{
	int len;
	char tftp_ip[OPTION66_TFTP_IP_LEN];
}OPTION66_CFG;
#endif
...
#ifdef MODULE_OPTION66
static inline int getTftpParamFromOption66(uint8_t *option, int len, OPTION66_CFG*option66Cfg)
{
    int ret =  -1;
    if ((NULL == option) || (len <= 0) || (NULL == option66Cfg))
    {
        return -1;
    }
	if(len <= OPTION66_TFTP_IP_LEN)
	{
		memset(option66Cfg->tftp_ip, 0, OPTION66_TFTP_IP_LEN);
		memcpy(option66Cfg->tftp_ip, option, len);
		option66Cfg->tftp_ip[len] = '\0';
		option66Cfg->len = len;
...
#ifdef MODULE_OPTION66
	  if((option66 = get_option(packet, DHCP_OPTION66_TFTP_IP)) != NULL)
	  {
	  	len = (int)*(option66 -1);
		ret = getTftpParamFromOption66(option66, len, &info->option66cfg);
		if(ret < 0 )
		{
			msglogd (LOG_INFO, LOGTYPE_DHCP, "parse tftp parameters from option(66) fail");
		}else
			{
				msglogd (LOG_INFO, LOGTYPE_DHCP, "get tftp paramteters from  option(66) successfully");
			}
	  }
#endif
...
#if MODULE_DHCP_REQUEST_SAME_IP/*  by huangwenzhong, 22Apr13 */
        //last_ip = info->ip;
        /*changed by ZQQ, if info->ip = 0, not save it to last_ip becase it does not send DHCP Discover with option 50 when last_ip equal 0*/
        if (0 != info->ip) 
        {
        last_ip = info->ip;
            msglogd(LOG_INFO, LOGTYPE_DHCP,"Mark the ip %x for option 50.", last_ip);
        }
#endif
    }
    //msglogd (LOG_INFO, LOGTYPE_DHCP, "####tftp OPTION IP addr len:%d,:%s", info->option66cfg->len,info->option66cfg->tftp_ip);
