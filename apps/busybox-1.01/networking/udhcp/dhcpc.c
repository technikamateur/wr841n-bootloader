...
#define     DISCOVER_INVERT_TIMES   3
			...
			case REQUESTING:
				if (packet_num < 3) {
					...
					{
#ifdef FEATURE_ALWAYS_BROADCAST_DHCP_REQUESTED_PACKET
 ...
 */                    
            ...
            case DHCPC_MAC_CLONE: /* WAN MAC clone */
                ...
                {
					/*added by ZQQ, 2015.8.19, when re-read the MAC, update the client ID(option 61) at the same time.*/
					if (NULL != client_config.clientid)
					{
						memcpy(client_config.clientid + 3, client_config.arp, 6);
					}
					/*end add*/
