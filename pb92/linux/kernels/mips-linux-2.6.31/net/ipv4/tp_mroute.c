...
int lan_heard_igmp_leave(struct sk_buff *skb, __u32 group)
{
    ...
    if (mcp)
    {
    	if (skb->mark == FLAG_WLAN_BAND24G_AP1)/*无线成员退出，但组内仍然有其他无线成员*/
		{
			NULL;
		}
		else if (skb->mark == FLAG_IGMP_WLAN_NO_MEMBER)/* 无线成员全部退出 */
		{
			mcp->membership_flag &= (~FLAG_WLAN_BAND24G_AP1);
		}
		else/*有线成员退出*/
		{
			//modify port infomation
...
/* added by HouXB, 03Dec10 */
    ...
    if (mcp)
    {
        skb->mark = mcp->membership_flag | FLAG_IGMP_DATA_WAN2LAN;
