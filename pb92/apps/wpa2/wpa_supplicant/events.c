...
req_scan:
	...
	{
		/* wait more time after each scan, reduce influence to ath0. */
		if (wpa_s->scan_res_tried > 1 && wpa_s->scan_res_tried < 6)
		wpa_s->scan_res_tried ++;
		else if (wpa_s->scan_res_tried > 6)
		wpa_s->scan_res_tried = 6;
		timeout = 4 * wpa_s->scan_res_tried;
	}
